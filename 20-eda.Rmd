...
engineer 3 = count the number of games by developer


add a column that will count the number groups in a column
```{r}
...
promote_rel_dept1<-function(df){
  df %>% 
  assert(in_set(c("department","years_since_last_promotion"))) %>% 
  group_by(department) %>% 
  mutate(avg_yr_prom_dept=mean(years_since_last_promotion), na.rm=TRUE) %>% 
  select(avg_yr_prom_dept,department, everything()) %>% 
  ungroup() %>% 
  arrange(department) 
}
```
